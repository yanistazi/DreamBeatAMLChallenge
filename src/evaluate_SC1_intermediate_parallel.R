...
if(identical(predictorElasticNet,mypredictor)){
    ...
    result <- evaluateCV(mypredictor, celllines, response, alpha=alpha, nrepeats=nrepeats, nfolds=nfolds, mc.cores=mc.cores)

}else if (identical(predictorRF,mypredictor)){
    
    result <- evaluateCV(mypredictor, celllines, response, nntree=ntree, nodesize=nodesize, nrepeats=nrepeats, nfolds=nfolds, mc.cores=mc.cores)

}else if (identical(predictorXGBoost,mypredictor)){
    
    result <- evaluateCV(mypredictor, celllines, response, max.depth=max_depth, eta=eta, nthread=nthread, nrounds=nrounds, nrepeats=nrepeats, nfolds=nfolds, mc.cores=mc.cores)
    
}else if (identical(predictorkNNReg,mypredictor)){
    
    result <- evaluateCV(mypredictor, celllines, response, k=k, nrepeats=nrepeats, nfolds=nfolds, mc.cores=mc.cores)
    
}else if (identical(predictorSVR,mypredictor)){
    
    result <- evaluateCV(mypredictor, celllines, response, nrepeats=nrepeats, nfolds=nfolds, mc.cores=mc.cores, kernel=kernel)
}
